...
export class CompanyController {
  ...
  async addVenue(req: Request, res: Response): Promise<void> {
    try {
      ...
      const {name, type, description, capacity, address, phone, city, state} = req.body;
      const files = req.files as Express.Multer.File[];

      const imagePaths = files?.map((image) => image.path);

      const response = await this._addVenueUseCase.execute({name, type, description, capacity, address, phone, city, state, images: imagePaths })
