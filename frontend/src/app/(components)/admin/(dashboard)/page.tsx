...
export default function Page() {
  ...
  const { data: dashboardData, error: dahsboardFetchError } = useGetAdminDashboardDetailsQuery(undefined);
      ...
      <div className="flex mt-[64px]">
        ...
        <div className="flex-1 p-4 space-y-5 bg-slate-100 sm:p-6 md:space-y-6 lg:p-8 mt-5">
          <div className="flex items-center justify-center gap-4 sm:space-x-1 md:space-x-6 lg:space-x-8">
            <RevenueCard title={'Total Bookings'} data={dashboardData?.totalBookings} />
            <RevenueCard title={'Month Revenue'} data={dashboardData?.monthlyRevenueShare} />
            <RevenueCard title={'Year Revenue'} data={dashboardData?.yearlyRevenueShare} />
            <RevenueCard title={'Overall Revenue'} data={dashboardData?.overallRevenueShare} />
          </div>

          <CardWithTab data={dashboardData} />
          <div className="flex flex-wrap gap-3 sm:gap-6 lg:gap-10 justify-center">
          <BarChart dataKeys={['name','guests','events','revenue']} title="Top 3 Venues" data={dashboardData?.top3VenueData} />
          <BarChart dataKeys={['location','venues','events','revenue']} title="Top 3 Locations" data={dashboardData?.top3LocationData} />
